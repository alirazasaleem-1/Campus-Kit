...
def generate_semester_summary_pdf(filepath, student_name, roll_number, university,
    ...
    c.drawCentredString(width / 2, height - 175, "SEMESTER SUMMARY")

    c.setFillColor(navy)
    c.setFont("Helvetica-Bold", 24)
    c.drawCentredString(width / 2, height - 210, semester)

    # Student info
    info_y = height - 260
    c.setFillColor(soft)
    c.roundRect(70, info_y - 30, width - 140, 50, 10, fill=True, stroke=False)

    c.setFillColor(navy)
    c.setFont("Helvetica-Bold", 11)
    c.drawString(85, info_y - 10, f"Student: {student_name}")
    c.drawString(300, info_y - 10, f"Roll No: {roll_number}")

    # Course table
    table_y = info_y - 70
    col_widths = [200, 80, 80, 100]
    headers = ["Course Name", "Credits", "Grade", "Grade Points"]
    table_x = 70

    # Table header
    c.setFillColor(navy)
    c.roundRect(table_x, table_y, sum(col_widths), 30, 6, fill=True, stroke=False)

    x = table_x
    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 10)
    for i, header in enumerate(headers):
        c.drawCentredString(x + col_widths[i] / 2, table_y + 10, header)
        x += col_widths[i]
    ...
    for index, course in enumerate(courses):
        c.setFillColor(colors.white if index % 2 == 0 else colors.HexColor("#F1F5F9"))
        c.rect(table_x, current_y, sum(col_widths), 28, fill=True, stroke=False)
        ...
        values = [course["name"], str(int(credits)), course["grade"], f"{points:.1f}"]
        x = table_x
        c.setFillColor(navy)
        for i, value in enumerate(values):
            val_size = fit_text(c, value, col_widths[i] - 14, "Helvetica", 10)
            c.setFont("Helvetica", val_size)
            c.drawCentredString(x + col_widths[i] / 2, current_y + 9, value)
            x += col_widths[i]
    ...
    gpa_y = current_y - 20
    c.setFillColor(blue)
    c.roundRect(table_x, gpa_y, sum(col_widths), 40, 10, fill=True, stroke=False)

    c.setFillColor(colors.white)
    c.setFont("Helvetica-Bold", 14)
    c.drawCentredString(width / 2, gpa_y + 13, f"Semester GPA: {gpa}")

    # Footer
    c.setFillColor(slate)
    c.setFont("Helvetica-Oblique", 8)
    c.drawCentredString(width / 2, 40, "Generated by CampusKit")
...
@app.route("/semester-summary", methods=["GET", "POST"])
def semester_summary():
    if request.method == "POST":
        ...
        courses = []
        for i in range(1, 11):
            course_name = request.form.get(f"course_{i}", "").strip()
            credit_hours = request.form.get(f"credits_{i}", "").strip()
            grade = request.form.get(f"grade_{i}", "").strip()

            if course_name and credit_hours and grade:
        ...
        if not courses:
            return error_response("Please enter at least one course.")

        cleanup_old_files(DOWNLOAD_FOLDER)
        filename = f"semester_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        filepath = os.path.join(DOWNLOAD_FOLDER, filename)

        generate_semester_summary_pdf(
            filepath, student_name, roll_number, university,
            department, semester, courses
        )
    ...
    return render_template("semester_summary.html")
